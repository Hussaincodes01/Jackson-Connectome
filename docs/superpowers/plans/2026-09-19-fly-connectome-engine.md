...
Watch the three numbers yourself. Expect **240-275 steps/s** (measured range on the target RTX 2050; the earlier "over 1,000" figure was a prediction that ignored kernel-launch latency and is wrong), a spike fraction in the low single-digit percent, and VRAM well under 4,000 MB — around 184 MB in practice. Throughput varies with GPU boost clocks and any other process using the card, so run `nvidia-smi` in a second terminal to check nothing else is competing before you judge a number.
